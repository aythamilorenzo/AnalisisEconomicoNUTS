...
```{r, fig.asp = 1.1}
data.es <- data %>%
  filter(na_item == "B6N" & unit == "EUR_HAB" & substr(geo,1,2) == "ES" & nchar(geo) == 4 | geo == 'ES')
...
ggplot(data.top10, aes(x = reorder(full_name, values), y = values, fill = values)) +
  ...
  labs(
    title = "Top 10 países europeos con mayor renta neta disponible",
...
ggplot(data.top10PPS, aes(x = reorder(full_name, values), y = values, fill = values)) +
  geom_col(width = 0.7, show.legend = FALSE) +          # Barras horizontales y gradiente
  geom_text(aes(label = scales::comma(round(values,0))), 
            hjust = -0.1, size = 4, color = "black") +  # Etiquetas sobre las barras
  coord_flip() +
